...
class PrintInOrder {
  // print the items in a Tree T using an iterator
  constructor(T) {
    this._iter = T.keys();
  }
  print() {
...
class Print extends EulerTour {
  // Print the items in a Binary Search Tree using the Euler Tour Template
  visitExternal(T, v, result) {
    result[1] = "";
  }
  visitPostOrder(T, v, result) {
    result[1] =
      "(" + result[0] + "L, " + v.element().key() + " ,R" + result[2] + ")";
  }
  print(T) {
    if (T.size() > 0) {
...
function heightHelper(T, p) {
  // your code goes here Hint: you need to define this helper function
  if (T.isExternal(p)) {
    return 0;
  } else {
